...
负责本 Skill 所定义的 Godot GDScript 能力边界，不负责其他 Skill 的专属实现。

# Description

负责 Godot GDScript 编码规范、脚本结构、类型与可维护性约束。

# Purpose

为 Agent 提供明确、可发现、可验证的 Godot GDScript 能力边界。
...
当任务涉及 Godot GDScript 时，先读取本 Skill，再根据 Workflow 执行与验证。
