...
负责本 Skill 所定义的 Godot Animation Visual 能力边界，不负责其他 Skill 的专属实现。

# Description

负责动画、视觉表现、动画状态与表现层组织。

# Purpose

为 Agent 提供明确、可发现、可验证的 Godot Animation Visual 能力边界。
...
- 用户任务明确涉及 Godot Animation Visual。
...
当任务涉及 Godot Animation Visual 时，先读取本 Skill，再根据 Workflow 执行与验证。
