...
def main():
    ...
    for kind,path in files.items():
        data, errors=load_config(path, kind)
        cfg[kind]=data
        if errors:
            structurally_invalid.add(kind)
            for error in errors:
                r.fail(error)
    template=read_text(template_path,r)
    ...
    # Validate the machine-readable Skill Loading contract
    # and semantic loading policy.
    if "loading" not in structurally_invalid:
        ...
        for policy,expected in policy_contracts.items():
            ...
            behavior_raw=block.get("behavior",{}) if isinstance(block,dict) else {}
            try:
                behavior=normalize_behavior(behavior_raw)
            except Exception as exc:
                r.fail(f"skill-loading.yaml: policies.{policy}.behavior is invalid: {exc}")
                continue
    ...
    # Validate the machine-readable Skill Collaboration contract
    # against the canonical taxonomy and workflow categories.
    if "collaboration" not in structurally_invalid:
